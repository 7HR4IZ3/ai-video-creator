...
def download_video(url, path, filename):
    ...
    try:
        ...
        yt = YouTube(url,
                     ...
                     on_complete_callback=on_complete) # Add callbacks
        ...
        if not stream:
            print("No progressive MP4 stream found. Trying highest resolution overall progressive stream.")
            stream = yt.streams.filter(progressive=True).order_by('resolution').desc().first() # Fallback to any progressive

        if not stream:
             print("Error: Could not find any suitable *progressive* stream (video+audio).")
        ...
        print(f"Selected Stream: Resolution={stream.resolution}, Type={stream.mime_type}, File Size={stream.filesize_mb:.2f} MB")
    ...
    # Specific Pytube errors
        ...
        print(f"Error: The video at {url} is unavailable (private, deleted, region-locked, etc.).")
    except RegexMatchError:
        print(f"Error: Could not parse the YouTube URL: {url}. Please check if it's valid.")
...
def on_progress(stream, chunk, bytes_remaining):
    ...
    print(f"\rDownloading... {percentage:.1f}% completed ({bytes_downloaded / (1024*1024):.2f}MB / {total_size / (1024*1024):.2f}MB)", end='')

def on_complete(stream, file_path):
    """Callback function for download completion."""
    print(f"\nDownload complete! Video saved as: {file_path}") # Newline after progress indicator

# --- Execution ---
if __name__ == "__main__":
    if video_url == "YOUR_YOUTUBE_VIDEO_URL_HERE":
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!! ERROR: Please replace 'YOUR_YOUTUBE_VIDEO_URL_HERE' in the script     !!!")
        print("!!!        with an actual YouTube video URL before running.               !!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    else:
